...
export default function PlansClient() {
  ...
  const planExecutionChatId = plan?.executionChatId ?? null;
  useEffect(() => {
    if (!activeProjectId || !plan) {
    ...
    const checkActive = async () => {
      try {
        ...
        const isThisPlan = !!(req?.active
          ...
          && planExecutionChatId
          && req.sessionId === planExecutionChatId);
    ...
    const stopStarted = window.electronAPI?.project?.onAgentStarted?.((event) => {
      ...
      if (planExecutionChatId && event.sessionId === planExecutionChatId) {
    ...
    const stopChatToken = window.electronAPI?.p2p?.onChatToken?.((event: { projectId?: string; scope?: string; sessionId?: string | null }) => {
      if (!planExecutionChatId) return;
      if (event.projectId && event.projectId !== activeProjectId) return;
      if (event.scope !== "solo-chat") return;
      if (event.sessionId !== planExecutionChatId) return;
      setAgentRunning(true);
    ...
    const stopChatMessage = window.electronAPI?.p2p?.onChatMessage?.((event: { projectId?: string; scope?: string; conversationId?: string; message?: { id?: string } }) => {
      ...
      if (event.scope !== "solo-chat") return;
      if (typeof event.conversationId === "string" && event.conversationId === `solo-${planExecutionChatId}`) {
  ...
  }, [activeProjectId, plan, planExecutionChatId]);
