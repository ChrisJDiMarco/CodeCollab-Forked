...
function createP2PService({ sharedStateService, sendEvent: initialSendEvent }) {
  ...
  function generateInviteCode(remoteUrl, projectName, secret, options = {}) {
    ...
    //
    // Default expiration: 7 days. Callers can override via options.ttlMs (set
    // to 0 / null to disable expiration). Short-lived invites limit the blast
    // radius if a code leaks via screenshots, chat history, or email.
    const payload = { v: 3, r: remoteUrl, n: projectName || "Project", s: secret };
    const ttlMs = (options.ttlMs === null || options.ttlMs === 0)
      ? 0
      : (typeof options.ttlMs === "number" && Number.isFinite(options.ttlMs) && options.ttlMs > 0
          ? options.ttlMs
          : 7 * 24 * 60 * 60 * 1000);
    if (ttlMs > 0) payload.exp = Date.now() + ttlMs;
  ...
  function decodeInviteCode(code) {
    try {
      ...
      // Strict remote URL validation — reject anything containing shell
      // metacharacters, whitespace, or control characters before the URL is
      // ever passed to git/clone. Even though we always invoke git via
      // execFile-style argv arrays, defense-in-depth: reject obviously
      // malicious URLs at the protocol boundary.
      const url = payload.r;
      if (url.length > 1024) {
        throw new Error("Invalid invite code - remote URL too long.");
      }
      const urlOk = /^(https?:\/\/|git@|ssh:\/\/)[A-Za-z0-9._@:/\-+~%]+(\.git)?$/.test(url);
      if (!urlOk) {
        throw new Error("Invalid invite code - unsupported or unsafe remote URL.");
      }
      ...
      // Optional expiration. Issued by recent hosts (v3+); older invites omit
      // `exp` and are accepted indefinitely so existing collaborations keep working.
      if (typeof payload.exp === "number" && Number.isFinite(payload.exp)) {
        if (Date.now() > payload.exp) {
          throw new Error("Invite code expired. Ask the host to generate a new one.");
        }
      }
      return {
        remoteUrl: url,
  ...
  /**
   * Rotate a project's P2P shared secret. Disconnects all current peers
   * (kicking anyone who still has the old secret), generates a fresh secret,
   * and rejoins the swarm under the new authenticated topic.
   *
   * The caller is responsible for persisting the new secret to settings AND
   * regenerating any outstanding invites — old invites become unusable on
   * the next attempt.
   *
   * Returns the new secret (base64url) so the caller can persist it.
   */
  async function rotateProjectSecret(projectId) {
    const existing = sessions.get(projectId);
    const newSecret = generateProjectSecret();
    if (!existing) {
      // Nothing to rotate live; caller still needs the new secret persisted.
      return { rotated: false, secret: newSecret };
    }
    const { repoPath, remoteUrl, member } = existing;
    await leaveProject(projectId);
    await joinProject(projectId, repoPath, remoteUrl, member, { secret: newSecret });
    return { rotated: true, secret: newSecret };
  }
